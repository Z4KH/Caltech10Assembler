...
def hex_offset(offset, symbols, memory, bytes_table, port=False):
    """
    ...
    warning = False
    error = False
    if memory == True and offset[:2].lower() != '0x' and offset != '' and offset and offset not in symbols:
        if port == False and offset not in bytes_table or port == True:
            error = True
    if offset == '':
        offset = '00'
        if memory == True:
