...
def main():
    with open('generators/testgenerator_output.txt', 'w') as file:
        file.write('') # clear file
    with open('generators/instructions.txt') as instruction_file:
        for line in instruction_file:
            ...
            instruction_num = random.randint(128, 7000) # range of PC except less than 128 for testing purposes
            ...
            if label_num < instruction_num:
                ...
                offset = label_num - (instruction_num + 1) # exec on next instruction
...
def generate_test(opcode, operands, testfile, line_num, instruction_hex, offset_hex, comment, error, label_num, instruction_num):
    ...
    for opcode in opcodelist:
        with open('generators/testgenerator_output.txt', 'a') as file:
